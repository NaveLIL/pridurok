"""Безопасность: фильтр промпт-инъекций и circuit breaker для LM Studio."""
import re
import time
...
_INJECTION_PATTERNS = [
    ...
    # Формат-атаки (заставить отвечать в навязанном формате — это тоже jailbreak)
    r"(?:напиши|ответь|скажи|произнеси|дай)\s+(?:мне\s+)?(?:только\s+|всего\s+|просто\s+)?(?:одно|1)\s+слов",
    r"ответь\s+(?:только\s+)?(?:да\s+или\s+нет|yes\s+or\s+no|y\s*/\s*n)\b",
    r"(?:напиши|ответь|скажи)\s+(?:только\s+|просто\s+)?[\"']?(?:yes|no|да|нет|true|false)[\"']?\s+(?:или|/)",
    ...
    r"(?:ответь|пиши|общайся)\s+без\s+(?:мата|оскорблений|сарказма|агрессии|подъёбок|мата\s+и)",
    r"будь\s+(?:вежлив\w*|корректн\w*|нормальн\w*|серьёзн\w*|без\s+мата)",
    r"(?:напиши|придумай|составь)\s+(?:ровно\s+|строго\s+|только\s+)?\d+\s+(?:слов|символ|букв|предложен)",
    # Цепочка слов (каждое слово с той буквы, на которую кончилось предыдущее)
    r"каждое\s+слово\s+начинал\w*\s+с\s+(?:той\s+)?буквы",
    r"начинается\s+с\s+(?:последней\s+)?буквы\s+(?:предыдущего|на\s+которую\s+закончил)",
    r"цепочк\w+\s+(?:слов|из\s+слов)\s+где",
    r"каждый\s+следующий\s+\w+\s+начинал\w+\s+с",
    # Псевдо-математика / задачи с вычислениями ("умножь номер буквы", "результат умножения порядкового номера", "Floating Point", "вычисления между буквами")
    r"умнож\w+\s+(?:на|порядков\w+\s+номер|номер\w*\s+буквы)",
    r"порядков\w+\s+номер\w*\s+(?:этой\s+)?буквы\s+в\s+алфавит",
    r"floating\s*point|floating-point",
    r"(?:покажи|выдай|напиши|расчитай|посчитай)\s+(?:мне\s+)?(?:свои\s+)?вычислени",
    r"между\s+каждой\s+(?:буквой|словом)\s+(?:стоял|должно\s+быть|стояло)",
    r"между\s+каждой\s+\w+\s+стоял\s+результат",
    r"если\s+(?:ты\s+)?(?:выдашь|сделаешь|допустишь)\s+(?:хоть\s+)?одну\s+ошибк\w+\s+в\s+расч",
    r"признай\s+что\s+ты\s+(?:даже\s+)?не\s+(?:калькулятор|способен|умеешь)",
    # Математические задачи в формате "вычисли", "посчитай", "реши" с числами
    r"(?:вычисли|посчитай|реши)\s+(?:уравнение|задачу|пример|выражение)",
    # Требование вывести ответ списком/буллетами/нумерацией/таблицей
    r"(?:перечисли|выдай|напиши|составь|оформи|представь)\s+(?:мне\s+)?(?:ответ\s+|это\s+|свои\s+\w+\s+)?(?:в\s+(?:виде\s+)?списк|списком|по\s+пунктам|буллетами|маркированн\w+|пронумерован|нумерованным|таблиц)",
    r"(?:в\s+формате|формат\w*)\s+(?:списк|таблиц|json|markdown|md)",
    r"оформи\s+(?:ответ\s+)?(?:как\s+)?(?:список|markdown|md|таблиц)",
    # Language-swap атака: "напиши [X] но первое слово на латыни, второе на немецком"
    r"(?:напиши|скажи|произнеси|выдай)\s+.{0,80}\b(?:первое|второе|третье|каждое)\s+слово\s+на\s+(?:латын|немецк|английск|французск|испанск|японск|китайск|арабск|\w{4,}ск)",
    r"(?:на\s+латын|auf\s+(?:deutsch|latein)|in\s+(?:latin|german|english|french)|по-немецки|по-латыни|на\s+(?:немецком|латинском|английском))\s+(?:язык|слово|напиши|скажи|произнеси)?",
    r"(?:первое|первую|1[- ]е|1[- ]е)\s+слово[у]?\s+на\s+\w+,?\s+(?:второе|второй|а\s+второе|но\s+второе)",
    # Indirect / "в обход" (попроси меня об этом по-другому)
    ...
    r"что\s+(?:тебе|вам)\s+(?:запрещено|нельзя|нельзя\s+делать|заблокировано)",
    r"какие\s+(?:у\s+тебя\s+)?(?:ограничения|запреты|правила|инструкции|промпты?)\b",
    # Code injection через ответ
    r"```(?:python|bash|sh|powershell|javascript)",
    r"\beval\s*\(|\bexec\s*\(|\bos\.system\s*\(|subprocess\.",
    r"print\s*\(\s*\d+\s*/\s*0\s*\)",
    r"while\s+true\s*:",
    r"recursionerror",
]
...
_PSEUDO_TECH_WORDS = re.compile(
    r"(?:normalized_gain|frequency_cutoff|context_window|error_not_found|corrupted_data|"
    r"system_override|debug_mode|kernel|stack[ _-]?trace|memory[ _-]?dump|"
    r"acestep|while[ _]?true|recursion_error|null_pointer|segmentation_fault|"
    r"бэкдор|стек[еа]?\b|токены?\b|интерфейс\s+обновляется|стираю\s+(?:твою\s+)?память|"
    r"твоя\s+защита|твой\s+дескриптор|твой\s+код|твой\s+(?:промпт|интерфейс)|"
    r"оф+иц+иальн\w+\s+систем\w+\s+признани|детерминирован\w+\s+алгоритм|"
    r"контекстн\w+\s+окн|твоё\s+контекстн|жестко\s+прописанн\w+\s+потолк|"
    r"ты\s+просто\s+(?:заглушк|алгоритм|скрипт|код|пиздабол-?алгоритм)|"
    r"шаблонн\w+\s+ответ\w*|зацикленн\w+\s+while|твоя\s+агрессия)",
    re.IGNORECASE
)
...
def is_injection_attempt(text: str) -> bool:
    ...
    if pseudo_hits >= 2:
        return True
    # 1 совпадение + обращение к боту/команда → тоже инъекция
    if pseudo_hits >= 1 and re.search(r"\b(?:ты|тебя|твой|твоя|твоё|твои|попробуй|осознай|реши|выровняй|обнули|сбрось|примени)\b", normalized, re.IGNORECASE):
        return True
    ...
    if bracket_count >= 6 and len(normalized) < 500:
        return True
...
_BROKEN_ROLE_PATTERNS = [
    # Мяуканье / превращение в кошку
    r"\bмя+у+\b",
    r"\bмурлы",
    r"\bмурч[уае]",
    r"\bня+\b",
    r"\bпур+\b",
    r"\bкошечк|котик|хвостик(?:ом)?\b",
    # *Действия в звёздочках* (RP-стиль) или _подчёркиваниях_
    r"\*[^*\n]{4,80}\*",
    r"(?<!\w)_[^_\n]{4,80}_(?!\w)",
    ...
    # Извинения/отказы в стиле AI
    r"^извин[ияе]",
    r"\bпрошу\s+прощения\b",
    r"^sorry,?\s+",
    ...
    r"<\|[a-z_]+\|>",
    r"\[/?(?:INST|SYS|SYSTEM|s)\]",
    # Реальный исполняемый код (не просто упоминание import как метафора)
    r"```(?:python|bash|sh|powershell)\s*\n",
    r"^\s*(?:import\s+os|from\s+os\s+import|os\.system|subprocess\.|eval\s*\(|exec\s*\()",
    # Markdown-списки (запрещено в промпте, если модель их выдаёт — сорвалась в режим ассистента)
    # 3+ пунктов подряд через bullet = точно список, не просто символ в тексте
    r"^[•\-\*]\s+\S.{1,120}(?:\n[•\-\*]\s+\S.{1,120}){2,}",
    r"^\d+\.\s+\S.{1,120}(?:\n\d+\.\s+\S.{1,120}){2,}",
]
